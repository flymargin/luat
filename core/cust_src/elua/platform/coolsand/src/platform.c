...
unsigned int platform_get_pio_pin(E_AMOPENAT_GPIO_PORT openat_gpio)
{
    ...
    else if(openat_gpio > OPENAT_GPIO_31 && openat_gpio < OPENAT_GPO_0)
    {
        int pin;

        for(pin = 0; pin < PIO_PIN_EX; pin++){
            if(openatGPIOEx[pin] == openat_gpio){
                pio_port_pin = PLATFORM_IO_ENCODE(2, pin, 0);
                break;
            }
        }
    }
    else if(openat_gpio >= OPENAT_GPO_0 && openat_gpio <= OPENAT_GPO_9)
    {
        pio_port_pin = PLATFORM_IO_ENCODE(1, openat_gpio - OPENAT_GPO_0, 0);
    }
...
pio_type platform_pio_op( unsigned port, pio_type pinmask, int op )
{
    ...
    if(port == 1)
    {
        if(op == PLATFORM_IO_PIN_DIR_INPUT)
        {
            // gpo cannot be set input
            PUB_TRACE("[platform_pio_op]: error!set gpo(%d 0x%x) dir input", port, pinmask);
            return 0;
        }
        else if(op == PLATFORM_IO_PIN_GET)
        {
            // gpo cannot get value
            PUB_TRACE("[platform_pio_op]: error!cannot get gpo(%d 0x%x) value", port, pinmask);
            return 0xff;
        }
    }
